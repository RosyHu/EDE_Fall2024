...
```{r project.setup, warning=FALSE, include=FALSE}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
The raw dataset has `r nrow(epa_data)` rows and `r ncol(epa_data)` columns
---
...
summary_PM25 <- epa_data %>% 
  drop_na(PM2.5) %>% 
  group_by(COUNTY) %>% 
  summarize(`"$\\mu g/m^3$"` = round(mean(PM2.5, na.rm = TRUE),2))%>% 
  arrange(`"$\\mu g/m^3$"`)
#summary_PM25 
...
summary_Ozone <- epa_data %>% 
  drop_na(Ozone) %>% 
  group_by(COUNTY) %>% 
  summarize(`"$\\mu g/m^3$"` = round(mean(Ozone, na.rm = TRUE), 2)) %>% 
  arrange(`"$\\mu g/m^3$"`)
#summary_Ozone

#Use the `kable` function to produce the 2 tables in markdown format
knitr::kable(summary_PM25, caption = "Mean of PM25")

knitr::kable(summary_Ozone, caption = "Mean of Ozone")
```
...
```{r ozone_2018_plot, echo=FALSE, fig.align='left', fig.cap="Ozone Levels in 2018"}
# Filter data in 2018
ozone_2018 <- epa_data %>%
  filter(year(Date) == 2018)

#Box plot for Ozone levels by month in 2018
ggplot(ozone_2018, aes(x = factor(month(Date)), y = Ozone))+
  geom_boxplot(fill = "lightblue", color = "darkblue")+
  labs(
    title = "Ozone Levels by Month in 2018",
    x = "Month",
    y = "Ozone Level"
  ) +
  theme_minimal()
```
```{r ozone_2019_plot, echo=FALSE, fig.align='left', fig.cap="Ozone Levels in 2019"}
...
ggplot(ozone_2019, aes(x = factor(month(Date)), y = Ozone)) +
  ...
  labs(
    title = "Ozone Levels by Month in 2019",
    x = "Month",
    y = "Ozone Level"
  ) +
  theme_minimal()
...
## Git Repository
https://github.com/RosyHu/EDE_Fall2024.git
